...
contract VideoStorage {
    ...
    // Checking if a videoHash already exists on the blockchain
    function checkExistingVid(string memory videoHash) public view returns (bool) {
        return bytes(videoResults[videoHash].videoHash).length != 0;
    }

...
function VideoUpload() {
  ...
  const uploadVideo = async (file, result) => {
    ...
    try{
      ...
      if (error.message.includes('reverted with reason string')) {
        ...
        if (reason === 'Video already analyzed and exists on blockchain.') {
            const endTime2 = new Date().getTime();
            console.log('Time taken for retrieving the existing video:', endTime2 - startTime);
  ...
  // const checkVideoHash = async (hash) => {
  //   if(!contract){
  //     console.error('Contract not loaded');
  //     return;
  //   }
  //   try {
  //     const video = await contract.getVideo(hash);
  ...
  //       alert('An error occurred while fetching video.');
  //     }
  //   }
  // }
  ...
  const handleUpload = async () => {
    ...
    try {
      // check for already uploaded video to the blockchain before ml model testing
      // check for videos on the blockchain


      //checking the time for machine learning models to predict the video
      ...
      console.log('Time taken for machine learning model:', end1 - start1);

      const data = await response.json();
      setResult(data);

      //checking the time for uploading and retrieval of the vide on the blockchain
      ...
      console.log('Time taken for uploading and retrieval:', end - start);
